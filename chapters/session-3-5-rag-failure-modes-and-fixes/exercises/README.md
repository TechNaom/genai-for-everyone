...
or context stuffing. Your actual work happens in `starter.py`.
...
2. Open `starter.py` and fill in, for each case:
...
No API key and no internet access needed — `broken_pipeline.py` reuses
the same fully offline chunking/embedding/retrieval code from Sessions
3.3–3.4, run against the included `sample_handbook.pdf`.
...
For a sharper follow-up on Case 2 specifically — instead of just
confirming the correct chunk is missing from the top-3, digging into
exactly which words in the wrongly-retrieved chunks caused them to
outrank it — see the [project](../project/index.html).
...
Pro path answers, is in `solution.py` in this same folder. Try your own
diagnosis first — this is a genuinely useful skill to practice under
your own steam before checking against someone else's reasoning.
