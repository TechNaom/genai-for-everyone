"""
Session 4.2 — Reference Solution (Core Path)
...
no external calls required. run_conversation() and run_full_demo()
require a live ANTHROPIC_API_KEY -- run them yourself with your own key
to see your own live output, which may vary between runs since
tool-calling decisions are made by the model itself.

The Pro path (a third, deliberately ambiguous tool) is not part of this
core-path solution -- see the project/ folder for that extended build.
"""
