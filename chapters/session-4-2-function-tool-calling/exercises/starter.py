...
those right first -- they're most of what makes the loop actually work
once you do have a key. See exercises/README.md for the free/open vs.
paid-API distinction this session is built around.
...
def run_conversation(user_message: str, max_rounds: int = 5) -> str:
    ...
    If you don't have an API key yet, you can still exercise this
    function's *shape* by passing a fake key -- `anthropic.Anthropic()`
    will construct fine, and the request will only fail once it actually
    reaches the network. Reading the resulting error message is still a
    useful sanity check that your request is built correctly.
