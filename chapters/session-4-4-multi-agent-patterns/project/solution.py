...
This is the same baseline system as starter.py (writer + fact-checker +
style-checker + impact-checker, with convergence checking), PLUS a worked
version of the first "Extensions" challenge from README.md: a fourth
reviewer, audience_relevance_agent, checking whether the essay actually
speaks to its intended reader. Everything else -- the writer, the other
three reviewers, and the convergence logic -- is unchanged from starter.py.
...
def audience_relevance_agent(essay: str, topic: str) -> str:
    """
    Reviewer 4 (extension): Checks whether the essay actually speaks to its
    intended reader, not just whether it is well-written in the abstract.
    """
    prompt = f"""You are an audience-relevance reviewer. The essay below is about: {topic}

{essay}

Provide specific feedback on:
1. Does this essay assume knowledge or context an average reader may not have?
2. Does it address what someone reading about this topic would actually want to know?
3. Is anything included that's irrelevant to the target reader's likely concerns?
...
def multi_agent_multi_reviewer_loop(topic: str, max_revisions: int = 3, convergence_threshold: float = 0.7) -> dict:
    """
    Multi-reviewer loop with convergence checking, now with four reviewers
    (fact, style, impact, audience-relevance) instead of three.
    ...
    for revision_num in range(max_revisions):
        ...
        # Collect feedback from all four reviewers
        feedback_dict = {
        ...
        print("Checking audience relevance...")
        audience_feedback = audience_relevance_agent(draft, topic)
        feedback_dict["reviews"]["audience-relevance-checker"] = audience_feedback
        print(f"  → {audience_feedback[:80]}...\n")

        result["feedback_history"].append(feedback_dict)

        # Check convergence: are we getting the same feedback across all four reviewers?
        current_feedback_str = " ".join([fact_feedback, style_feedback, impact_feedback, audience_feedback])
        ...
        all_feedback = [
            ...
            {"reviewer": "Audience-Relevance-Checker", "feedback": audience_feedback},
        ]
