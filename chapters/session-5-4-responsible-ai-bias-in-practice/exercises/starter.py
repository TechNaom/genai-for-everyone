...
1. Load sample outputs (48 examples)
2. Audit for bias across 3 groups
3. Calculate metrics
4. Document findings
5. Suggest mitigations

This implementation is already complete and runnable -- study how each method
works, run it, then use the extension ideas in README.md to go further.
"""
...
class BiasAudit:
    ...
    def generate_report(self):
        ...
        if gap > 5:
            print("FINDING: Significant accuracy disparity")
        print()
        ...
        if rep_gap > 10:
            print("FINDING: Unequal representation")
        print()
...
# TODO (yours to add -- see README.md "Extend it" section):
#   - A 4th group ("asian" or another group of your choice) with its own
#     sample outputs, so the audit compares across 4 groups instead of 3.
#   - A `severity(gap)` helper that returns "HIGH"/"MEDIUM"/"LOW" instead of
#     just printing a bare FINDING line, matching the severity levels used
#     in the lesson's Part 3 audit report.
#   - A `save_report_json()` method that writes the three metrics dicts to a
#     JSON file, so the audit's numbers are machine-readable, not just printed.
