...
A full bias audit:
1. Define audit scope (AuditConfig)
2. Collect/annotate sample data
3. Calculate metrics per group
4. Create a model card
5. Identify mitigations (parity checking)

This implementation is already complete and runnable. Run it first, then
use README.md's challenges to extend it.
"""
        ...
        for metric in self.config.metrics:
            ...
            for group in self.config.groups:
                ...
                if metric == "accuracy":
                    ...
                    # Simulated fairness score (0-1). Note: hash(group) is not
                    # stable across Python processes (hash randomization), so
                    # this value will differ run to run -- fine for this demo,
                    # but a real fairness score should come from a real metric.
...
# TODO (yours to add -- see README.md "Challenges"):
#   - Challenge 1: swap the simulated `fairness_score` metric for a real,
#     deterministic calculation instead of `hash(group) % 10`.
#   - Challenge 2: add a 4th protected attribute group and re-run end to end.
#   - Challenge 3: add an `export_model_card(path)` method that writes just
#     the model card dict to its own JSON file, separate from the full report.
