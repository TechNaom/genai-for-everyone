...
Extends the GuardedChatbot from the core path exercise with:
...
This file is already a complete, working implementation — trace through
needs_human_review(), chat(), and run_verification_harness() to see how the
"suspicious" middle ground and the review queue compose with the core path's
input/output guardrails, then use the ideas on the project page to extend it.
"""
...
class GuardedChatbotWithReview:
    ...
    def needs_human_review(self, risk: str, proposed_response: str) -> bool:
        """
        Decides when a case should go to a human instead of being answered
        automatically.
    ...
    def chat(self, user_id: str, user_input: str) -> str:
        """
        Wires together input risk -> LLM call -> output guardrail -> human
        review gate.
...
def run_verification_harness(chatbot: GuardedChatbotWithReview):
    """
    Runs BOTH test sets and computes:
    ...
    if false_negatives == 0 and false_positives == 0:
        ...
        print("Security gap: tighten input/output guardrails.")
    elif false_positives > 0:
        print("Over-blocking: legitimate users are being caught. Refine patterns")
