...
-----------------------
1. PROMPT_TEMPLATE   — fill in the template with the four pillars, tone
                        examples, internal-reasoning instructions, and the
                        JSON schema. (See the TODOs below.)
2. build_prompt()    — fills the template's variables in.
3. call_llm()        — already provided (stub or real API call).
4. parse_reply()     — defensively parses + validates the model's JSON.
5. generate_reply()  — ties it all together: build -> call -> parse -> return.
...
#          the *style* of, not the content.
#
# TODO(2): Fill in the {reasoning_instructions} section. It should ask the
#          model to privately think through: what the customer actually
#          wants, whether the ticket gives enough information to resolve
#          it, and whether the issue involves money/data loss/repeated
#          complaints (which should push toward escalation) -- WITHOUT
#          including that reasoning in the final output.
#
# Everything else in the template is provided for you.
...
def build_prompt(ticket_subject: str, ticket_body: str, tone: str) -> str:
    ...
    a cheap guard that catches a bug before it ever reaches the model.
    """
