...
Pro path: add a feedback log and a drift check against a golden dataset.
...
def mock_service_call(user_input: str) -> Dict:
    """Simulates calling your Week 6.1 service; randomizes latency/tokens for realism."""
    time.sleep(0.01)
...
def log_feedback(request_index: int, thumbs_up: bool):
    """TODO 3 (Pro path): append {request_index, thumbs_up} to FEEDBACK_LOG."""
    raise NotImplementedError


def check_drift(score_fn, threshold: float = 0.8) -> bool:
    """
    TODO 4 (Pro path): re-score GOLDEN_DATASET using score_fn(input, expected) -> bool,
