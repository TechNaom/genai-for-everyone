...
2. **TODO(2)** — Write the internal-reasoning instructions: ask the model to
   think through the customer's actual need, how urgent the situation is,
   and whether escalation is warranted — *before* writing the final JSON,
   without leaking that reasoning into the output.
...
6. **TODO(6)** — Implement `generate_reply()` so a parsing failure becomes a
   safe escalation instead of a crash.
...
- If `call_llm()` returns something unparseable, `generate_reply()` still
  returns a valid `SupportReply` — with `escalate: True` — rather than
  crashing.

## A note on the offline stub

The provided `call_llm()` stub always returns the same fixed mock response,
regardless of what's in the prompt. That's intentional — it exists purely
to let you build and test `parse_reply()` and the fallback logic without
needing API access. If you run the full demo, you'll notice all three
sample tickets produce identical output; that's the stub, not a bug in
your code. Set `USE_REAL_LLM=1` with a real API key (TODO 4) to see
genuinely different, tone-appropriate replies per ticket.
...
less trustworthy your design feels even before you've run a single real LLM
call. That instinct (trustworthy vs. not) is the thing this whole week has
been training.

## Stuck?

A fully worked reference solution is in `solution.py`. Try to get through
TODO(5) on your own first — the defensive parsing logic is the part most
worth struggling with, since it's the part that quietly makes or breaks a
real production prompt system.
